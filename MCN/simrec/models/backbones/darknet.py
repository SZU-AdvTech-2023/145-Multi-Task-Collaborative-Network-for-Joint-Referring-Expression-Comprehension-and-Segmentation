...
class DarkNet53(nn.Module):
    def __init__(
        ...
        self.module_list.append(darknet_conv(in_ch=32, out_ch=64, ksize=3, stride=2))  # /2
        self.module_list.append(darknetblock(ch=64))
        self.module_list.append(darknet_conv(in_ch=64, out_ch=128, ksize=3, stride=2))  # /2
        self.module_list.append(darknetblock(ch=128, nblocks=2))
        self.module_list.append(darknet_conv(in_ch=128, out_ch=256, ksize=3, stride=2))  # /2
        self.module_list.append(darknetblock(ch=256, nblocks=8))  # shortcut 1 from here
        self.module_list.append(darknet_conv(in_ch=256, out_ch=512, ksize=3, stride=2))  # /2
        self.module_list.append(darknetblock(ch=512, nblocks=8))  # shortcut 2 from here
        self.module_list.append(darknet_conv(in_ch=512, out_ch=1024, ksize=3, stride=2))  # /2
