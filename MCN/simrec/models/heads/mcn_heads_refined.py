...
class MCNhead_refined(nn.Module):
    """
    ...
    def __init__(
        ...
        super(MCNhead_refined, self).__init__()
    ...
    def co_energe(self, x_map, y_map, x_attn, y_attn, eps=1e-6):
        ...
        # softmax obtain the energy distributions of REC and RES
        x_map = F.softmax(self.s_proj(x_attn).view(b, -1), -1)  # (b, h_x*w_x)
        y_map = F.softmax(self.d_proj(y_attn).view(b, -1), -1)  # (b, h_y*w_y)
    ...
    def forward(self, xin, yin, x_label=None, y_label=None, x_map=None, y_map=None, x_attn=None, y_attn=None):
        ...
        if not self.training:
            pred[..., :4] *= self.stride  # 将坐标尺度恢复到输入尺度  (b, 3, h, w, 5)
            pred = pred.view(batchsize, -1, n_ch)  # (b, 3*h*w, 5)
            # xc,yc,,w,h->xmin,ymin,xmax,ymax
            pred[:, :, 0] = pred[:, :, 0] - pred[:, :, 2] / 2
            pred[:, :, 1] = pred[:, :, 1] - pred[:, :, 3] / 2
            pred[:, :, 2] = pred[:, :, 0] + pred[:, :, 2]
            pred[:, :, 3] = pred[:, :, 1] + pred[:, :, 3]
            score = pred[:, :, 4].sigmoid()  # TODO 上面也经过一次sigmoid处理，两次sigmoid有点奇怪，虽然不影响最终score的排序结果    # (b, 3*h*w)
            max_score, ind = torch.max(score, -1)  # (b, )
            ind = ind.unsqueeze(1).unsqueeze(1).repeat(1, 1, n_ch)
            pred = torch.gather(pred, 1, ind)  # (b, 1, 5)
            # TODO
            mask = self.nls(pred_seg=mask, pred_box=pred.view(batchsize, -1), weight_score=max_score.view(batchsize, 1))
        ...
        for b in range(batchsize):
            ...
            # 计算真实框和预测框（3 * h * w 个）的框重合度
            pred_ious = bboxes_iou(
                pred[b].view(-1, 4), truth_box, xyxy=False)  # (3*h*w, 1)
            ...
            for ti in range(best_n.shape[0]):
                if best_n_mask[ti] == 1:
                    ...
                    a = best_n[ti]
                    obj_mask[b, a, j, i] = 1  # TODO 这里为何置为1 [让target框中心点所在的grid且面积IOU最大的anchor对应的output参与下面loss的计算]
                    tgt_mask[b, a, j, i, :] = 1
                    # 真实值 xc, yc 相对于特征图的坐标 ----> 相对于其xc, yc所在gird的坐标 【范围0-1】
                    # 真实值 w, h 相对于特征图的尺度 ----> 相对于单元grid对应IOU最大anchor的尺度 【范围 】
        ...
        output[..., np.r_[0:4, 5:n_ch]] *= tgt_mask  # 令非真实框中心点所在grid且面积IOU最大的anchor对应的output的 xc,yc,w,h 为0
        output[..., 2:4] *= tgt_scale  # TODO ？乘以一个相对与特征尺度的wh相关系数？
        # TODO 这两步貌似是多余的
        target[..., 4] *= obj_mask  # TODO
        target[..., np.r_[0:4, 5:n_ch]] *= tgt_mask
        target[..., 2:4] *= tgt_scale  # TODO ？乘以一个相对与特征尺度的wh相关系数？
