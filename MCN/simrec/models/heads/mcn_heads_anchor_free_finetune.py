...
class MCNhead_anchor_free_finetune(nn.Module):
    """
    ...
    def __init__(
        ...
        super(MCNhead_anchor_free_finetune, self).__init__()
    ...
    def forward(self, xin, yin, x_label=None, y_label=None, x_map=None, y_map=None, x_attn=None, y_attn=None):
        ...
        #  2.预测试时的asnls
        #  3.ignore_score
        #     3.1 修改obj_mask初始化为0，大于ignore_score时obj赋值为1,回归框中心点所在grid也赋值为1
        #     3.2 修改 pred_best_iou = (pred_best_iou < self.ignore_thre) 【后面想想和上面的改法是等价的】
        #     修改后，参与计算的anchor大大减少，det_loss大大减小，但det_IOU上不去
        #  4.L2-->soft L1
        # 1 卷积, assp+上采样
        ...
        if y_label is not None:
            loss_seg = nn.BCEWithLogitsLoss(reduction='sum')(mask,
                                                             y_label) / 640. / batchsize  # TODO 640是怎么来的，为什么要除以
        # TODO
