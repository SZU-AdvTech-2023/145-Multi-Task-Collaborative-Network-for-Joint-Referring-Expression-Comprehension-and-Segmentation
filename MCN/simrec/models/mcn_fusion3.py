...
class MCN(nn.Module):
    ...
    def forward(self, x,y, det_label=None,seg_label=None):
        ...
        # x[-1]=self.fusion_manner(x[-1] ,y['flat_lang_feat'])  # TODO 只有最后一层视觉特征与语言特征进行混合，尝试所有层都和语言进行混合
        ...
        bot_feats,seg_map,seg_attn=self.seg_garan(y['flat_lang_feat'],bot_feats)  # (b,c,h,w), (b, h*w), (b,c,h,w)
        top_feats,det_map,det_attn=self.det_garan(y['flat_lang_feat'],top_feats)  # (b,c,h,w), (b, h*w), (b,c,h,w)
