...
class RefCOCODataSet(Data.Dataset):
    ...
    def load_img_feats(self, idx):
        ...
        if self.dataset in ['refcoco','refcoco+','refcocog']:
            ...
            if self.refs_anno[idx]['data_source'] == "coco":
                img_path = os.path.join(self.image_path, "train2014", iid)
            elif self.refs_anno[idx]['data_source'] == "flickr":
                img_path = os.path.join(self.image_path, "flickr", iid)
            elif self.refs_anno[idx]['data_source'] == "vg":
                img_path = os.path.join(self.image_path, "VG", iid)
            elif self.refs_anno[idx]['data_source'] == "saiaprtc12":
                img_path = os.path.join(self.image_path, "refclef", iid)
        else:
        ...
        box = np.array([self.refs_anno[idx]['bbox']], dtype=np.float64)  # TODO 类型错误 # TODO  # [x1, y1, w, h]
        # TODO merge中的iid为“2329120.jpg”，字符串，需要判断并将其转为整形
        # iid = deepcopy(self.refs_anno[idx]['iid'])
        iid = self.refs_anno[idx]['iid']
        ...
        return image,mask,box,self.refs_anno[idx]['mask_id'],iid
