...
parser.add_argument('--data_root', type=str, default="./")  # contains refclef, refcoco, refcoco+, refcocog and images
...
print('\nAmong them:')
# 根据不同的数据集，以及不同的划分，确定这个数据集的划分方式，后续对每种划分方式按照自己所需数据格式内容要求进行“分门别类”，形成json文件。
...
def show_a_batch(batch_size):
    ...
    for i in range(batch_size):
        ...
        print(image.shape)
        # print(bboxs[i][0])
        cv2.rectangle(image, (int(bboxs[i][0]), int(bboxs[i][1])),
                      (int(bboxs[i][0] + bboxs[i][2]), int(bboxs[i][1] + bboxs[i][3])), 255, 3)
        cv2.putText(image,
                    str(sent['sent']),
                    (20, 20),
                    cv2.FONT_HERSHEY_SIMPLEX,
                    .9, (0, 255, 0), 2)
        if not os.path.exists('debug_vis'):
            os.mkdir('debug_vis')
...
def prepare_dataset(dataset, splits, output_dir, generate_mask=False):
    ...
    for split in splits:
        ...
        split_num = 0
        ll = 0
        ref_ids = refer.getRefIds(split=split)  # 根据 split 获取refs().p文件中相关的 refs
        print(split + '_size:', np.alen(ref_ids))
        for i in ref_ids:

            # ref_id = ref_ids[i]
            refs = refer.Refs[i]  # .p文件中的ref，其中包含[ref_id, sent_ids, sentences, image_id, file_name, category_id, ann_id, split]八种信息
            bboxs = refer.getRefBox(i)  # 获取bbox信息，其中包含[x, y, w, h]
            sentences = refs['sentences']  # 获取sentences信息，其中包含[[sent_id, sent, raw, tokens],[...],[...]]信息
            image_urls = refer.loadImgs(image_ids=refs['image_id'])[0]  # ref 对应的图像信息【1张】，其中包含[id, file_name, coco_url, flickr_url, height, width, data_captured, license]八种信息
            cat = cat_process(refs['category_id'])  # ref 对应的 cat_id
            image_urls = image_urls['file_name']  # ref 对应图像的文件名称【1张】
            if dataset == 'refclef' and image_urls in ['19579.jpg', '17975.jpg', '19575.jpg']:  # ！这3张图片都是异常的
                continue

            for sentence in sentences:  # 统计该split中句子的最长长度
                ll = max(ll, len(sentence["tokens"]))

            box_info = bbox_process(bboxs, cat, i)  # add segement id TODO
            # f.write(image_urls)
            # f.write(box_info)
            # f.write(' '+str(i))
            if split == 'train':  # train 中一个 ref 对应一张图片（一个 ref 包含多个 sentence）
                ann = {}
                ...
                ann['mask_id'] = i
                anns.append(ann)
            else:  # 非 train 中一个 sentence 对应一张图片
                for sent in sentences:
                    ...
                    ann['mask_id'] = i
                    anns.append(ann)
            if generate_mask:  # 保存 ref_id 对应的 mask 为npy格式文件，文件名为 ref_id
                np.save(os.path.join(output_dir, 'masks', dataset, str(i) + '.npy'),
                        refer.getMask(refs)['mask'])  # if need seg mask ,set it!
        ...
        print('max_len:', ll)
        # f.close()
    f = open(os.path.join(output_dir, 'anns', dataset, dataset + '.json'), 'w')
