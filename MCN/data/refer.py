...
class REFER:
    ...
    def getAnnIds(self, image_ids=[], cat_ids=[], ref_ids=[]):
        ...
        if len(image_ids) == len(cat_ids) == len(ref_ids) == 0:
            ...
            if not len(ref_ids) == 0:
                # ids = set(ann_ids).intersection(set([self.Refs[ref_id]['ann_id'] for ref_id in ref_ids]))  # 求交集
                ann_ids = set(ann_ids).intersection(set([self.Refs[ref_id]['ann_id'] for ref_id in ref_ids]))  # 求交集
    ...
    def getMask(self, ref):
        ...
        ann = self.refToAnn[ref['ref_id']]  # 根据 ref 中的 ref_id 获得 annotation
        image = self.Imgs[ref['image_id']]  # 根据 ref 中的 image_id 获得 图像信息
        ...
        m = m.astype(np.uint8)  # convert to np.uint8 图像大小的0-1掩码
        # compute area
        area = sum(mask.area(rle))  # should be close to ann['area']  # 有掩码的总像素数量
